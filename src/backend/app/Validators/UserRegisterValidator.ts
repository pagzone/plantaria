...
export default class UserRegisterValidator {
	static schema = z
		.object({
			...
			principal: z
				.string()
				.max(128, { message: "Principal too long" })
				.optional(),
			name: z
			...
			location: z
				...
				.max(60, { message: "Location too long" }),
		})
		.refine(
			(data) => {
				...
				return (
					(hasEmailAndPassword && !hasPrincipal) ||
					(!hasEmailAndPassword && hasPrincipal) ||
					(hasEmailAndPassword && hasPrincipal)
				);
			},
			{
				message:
					"Either provide email/password or log in with Internet Identity.",
				path: ["email"],
			},
		);
